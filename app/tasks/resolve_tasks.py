...
def task_resolve_alerts() -> None:
    """Авто-резолв активных событий, у которых последняя метрика
    больше не нарушает порог. Запускается периодически из Beat.
    """
    logger.info("Запуск auto_resolve_system_alerts")
    asyncio.run(_run())
