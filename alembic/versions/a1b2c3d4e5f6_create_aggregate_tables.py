...
down_revision: Union[str, Sequence[str], None] = '7c818dce7bd1'
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint('server_id', 'period_type', 'period_start', name='uq_metric_agg_server_period'),
    )
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint('id'),
        sa.ForeignKeyConstraint(['server_id'], ['servers.id'], ondelete='CASCADE'),
        sa.UniqueConstraint('server_id', 'container_name', 'period_type', 'period_start', name='uq_docker_agg_container_period'),
    )
