...
### Способ 1 — install-скрипт (Debian/Ubuntu, рекомендуется)

Сначала зарегистрируй сервер через UI (`Серверы → +`) или curl и сохрани `api_key` из ответа. Затем на удалённой машине:
...
Без env-переменных скрипт спросит URL и ключ интерактивно. Повторный запуск безопасен — конфиг `/etc/pulsewatch/agent.env` не перезаписывается целиком, только ключи `AGENT_API_URL` и `AGENT_API_KEY`. Опциональные настройки (например `AGENT_SEND_INTERVAL_SECONDS`) можно добавить туда вручную, скрипт их сохранит.

После установки логи — `journalctl -u pulsewatch-agent -f`.
...
sudo -u pulsewatch git clone https://github.com/IDRENAL/PulseWatch.git /opt/pulsewatch
cd /opt/pulsewatch
sudo -u pulsewatch python3 -m venv .venv
sudo -u pulsewatch .venv/bin/pip install -r agent/requirements.txt

# 3. Конфиг агента
sudo tee /etc/pulsewatch/agent.env > /dev/null <<EOF
AGENT_API_URL=http://192.168.0.4:8000
AGENT_API_KEY=1.<секрет_из_servers_register>
AGENT_SEND_INTERVAL_SECONDS=10
EOF
sudo chmod 600 /etc/pulsewatch/agent.env
sudo chown root:pulsewatch /etc/pulsewatch/agent.env
