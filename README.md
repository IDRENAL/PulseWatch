...
Тестов на фронтенд два уровня: unit-тесты бэкенда (pytest, 220+) и smoke-тесты UI через Playwright (см. ниже).
...
- Логи (journald) не сохраняются в БД — только live-stream через WebSocket. Соответственно CSV-экспорта логов нет, только событий (`/alerts/events/export`). Чтобы добавить — нужна таблица + retention policy.
- E2E-тесты покрывают только login/tabs/logout. Расширение (формы, графики, WS) — отдельная работа.
- Alertmanager-маршрутизация только одна — webhook на бэкенд. Если хочешь Slack/email-receiver напрямую от Alertmanager — правь `alertmanager.yml`.
- Frontend mobile-friendly но не отлажен на маленьких экранах.
